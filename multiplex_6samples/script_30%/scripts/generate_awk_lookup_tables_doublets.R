...
# qsub -I -V -l nodes=1:ppn=3,pmem=60gb,walltime=02:00:00 -A lp_expcardiology -m bea -M yile.fu@kuleuven.be -d "$PWD"
...
prop_doublets_sims <- 0.3

# dataset names
dataset_name_sims <- "IDCM"
...
f_sim_doublets <- function(prop_doublets, dataset_name, file_barcodes_merged) {
  ...
  dir_tmp <- paste0(dir_out, "/", dataset_name, "/6samples/doublets_sims/", prop_doublets * 100, "pc")
...
for (prop_doublets in prop_doublets_sims) {
  for (dataset_name in dataset_name_sims) {
    runtime <- system.time({
      file_barcodes_merged <- file.path(dir_out, dataset_name, "/6samples/barcodes_merged/barcodes_merged.tsv")
    ...
    fn_runtime <- file.path(
      paste0(dir_tmp, "/runtime_lookup_table_doublets_", dataset_name, "_", prop_doublets * 100, "pc_6samples.txt")
    )
    sink(fn_runtime)
    ...
    fn_timestamp <- file.path(
      paste0(dir_tmp, "/timestamp_lookup_table_doublets_", dataset_name, "_", prop_doublets * 100, "pc_6samples.txt")
    )
